...
(defcustom emigo-prompt-symbol "EmigoE> "
  "The prompt string used in EmigoE buffers."
  :type 'string
  :group 'emigo)
...
;; Window display functions moved to emigo-window.el
...
(defvar emigo-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c w") #'emigo-toggle-window-mode)  ;; Toggle window mode
    (define-key map (kbd "C-c s") #'emigo-switch-to-current-project)  ;; Manual session switch
